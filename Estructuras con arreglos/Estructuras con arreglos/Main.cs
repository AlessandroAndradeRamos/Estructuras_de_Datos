...
namespace Estructuras_con_arreglos
{
    public partial class frmMain : Form
    {
        ...
        private void btnBuscar_Click(object sender, EventArgs e)
        {
            ...
            {
                codigo = Convert.ToInt32(txtDatoCodigo.Text);
                txtRespuesta.Text = producto.Buscar(Convert.ToInt32(codigo), vectorPrincipal);
            }
            else
        ...
        {
            if (!string.IsNullOrEmpty(txtDatoNombre.Text) && !string.IsNullOrEmpty(txtDatoCodigo.Text) && !string.IsNullOrEmpty(txtDatoCantidad.Text)
                && !string.IsNullOrEmpty(txtDatoCosto.Text) && !string.IsNullOrEmpty(txtInsertar.Text))
            {
                ReadySet();
                producto = new Producto(nombre, codigo, cantidad, costo);
                posicion = Convert.ToInt32(txtInsertar.Text);
                producto.Insertar(producto, posicion - 1, vectorPrincipal);
                RayoMClean();
            }
            else
                MessageBox.Show("Todos los campos deben estar bien llenos", "AVISO", MessageBoxButtons.OK, MessageBoxIcon.Warning);
        }
